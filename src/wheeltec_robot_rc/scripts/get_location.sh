...
echo -e ""
echo -e "${Info} 控制 spark 移动到 ${Yellow_background_prefix}出发点${Font_color_suffix}\c"
echo && stty erase '^H' && read -p "       然后在这个界面内按回车..." 
rostopic pub /mark_nav std_msgs/String "data: 'learn start'" -1 >/dev/null
echo -e "${Info} 地点名为 ${Yellow_background_prefix}start${Font_color_suffix}"


echo -e ""
echo -e "${Info} 控制 spark 移动到 ${Yellow_background_prefix}人物点${Font_color_suffix}\c"
echo && stty erase '^H' && read -p "       然后在这个界面内按回车..." 
rostopic pub /mark_nav std_msgs/String "data: 'learn target'" -1 >/dev/null
echo -e "${Info} 地点名为 ${Yellow_background_prefix}target${Font_color_suffix}"
...
gnome-terminal -x bash -c "roslaunch turn_on_wheeltec_robot map_saver.launch -f $CURRENTPATH/../config/test_map"
